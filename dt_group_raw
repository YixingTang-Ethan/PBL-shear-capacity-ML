# -*- coding: utf-8 -*-
"""Decision Tree model with raw features and configuration-based validation.

This script trains and evaluates a Decision Tree regressor using the original
...
PARAM_GRID = {
    ...
    "max_depth": [4, 5, 6, 7, 8],
    "max_leaf_nodes": [16, 20, 24, 28, 32, None],
    "max_features": [4, 6, 8, 10, 13, None],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a raw-feature Decision Tree model."
    )
    parser.add_argument(
        "--input",
        type=str,
...
def build_model(random_seed: int) -> DecisionTreeRegressor:
...
def save_predictions(
    ...
    pd.concat([train_pred, test_pred], axis=0).sort_index().to_csv(
        output_dir / "dt_raw_main_group_split_predictions.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def main() -> None:
    ...
    print("\n=== Group-aware CV summary for best raw-feature Decision Tree ===")
    ...
    pd.DataFrame(grid_search.cv_results_).to_csv(
        output_dir / "dt_raw_groupaware_gridsearch_cv_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    with open(output_dir / "dt_raw_best_params.json", "w", encoding="utf-8") as f:
    ...
    print("\n=== Main group-split raw-feature Decision Tree result ===")
    ...
    ]).to_csv(output_dir / "dt_raw_main_group_split_metrics.csv", index=False, encoding="utf-8-sig")

    save_predictions(df, train_idx, test_idx, y_train_pred, y_test_pred, output_dir)

    with open(output_dir / "dt_raw_group_model.pkl", "wb") as f:
    ...
    repeat_df.to_csv(output_dir / "dt_raw_repeated_group_validation_metrics.csv", index=False, encoding="utf-8-sig")

    print("\nRepeated validation summary:")
    print(repeat_df[["R2", "RMSE", "MAE", "MAPE"]].describe())

    save_boxplot(repeat_df["R2"], "Decision Tree raw", "Test R²", output_dir / "repeated_group_validation_R2_boxplot")
    save_boxplot(repeat_df["RMSE"], "Decision Tree raw", "Test RMSE", output_dir / "repeated_group_validation_RMSE_boxplot")
