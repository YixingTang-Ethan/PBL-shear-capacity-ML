# -*- coding: utf-8 -*-
"""XGBoost model with train-only PCA and configuration-based validation.

This script trains and evaluates an XGBoost regressor for predicting the
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate an XGBoost model with train-only PCA."
    )
    parser.add_argument(
        "--input",
        type=str,
...
def save_predictions(
    ...
    pd.concat([train_pred, test_pred], axis=0).sort_index().to_csv(
        output_dir / "xgb_main_group_split_predictions.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def save_boxplot(values: pd.Series, ylabel: str, output_stem: Path) -> None:
    ...
    plt.boxplot([values.dropna()], labels=["XGBoost"])
    plt.ylabel(ylabel)
...
def main() -> None:
    ...
    print("\n=== Group-aware CV summary for best XGBoost pipeline ===")
    ...
    pd.DataFrame(grid_search.cv_results_).to_csv(
        output_dir / "xgb_groupaware_gridsearch_cv_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    with open(output_dir / "xgb_best_params.json", "w", encoding="utf-8") as f:
    ...
    print("\n=== Main group-split XGBoost result ===")
    ...
    ]).to_csv(output_dir / "xgb_main_group_split_metrics.csv", index=False, encoding="utf-8-sig")
    ...
    transformed_dataframe(best_pipeline, X_train).to_csv(
        output_dir / "X_train_transformed_for_SHAP.csv",
        index=False,
        encoding="utf-8-sig",
    )
    transformed_dataframe(best_pipeline, X_test).to_csv(
        output_dir / "X_test_transformed_for_SHAP.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    with open(output_dir / "xgb_group_pca_pipeline.pkl", "wb") as f:
    ...
    repeat_df.to_csv(output_dir / "xgb_repeated_group_validation_metrics.csv", index=False, encoding="utf-8-sig")
