# -*- coding: utf-8 -*-
"""XGBoost model with raw features and configuration-based validation.

This script trains and evaluates an XGBoost regressor using the original
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a raw-feature XGBoost model."
    )
    parser.add_argument(
        "--input",
        type=str,
...
def build_model(random_seed: int) -> XGBRegressor:
    return XGBRegressor(
...
def save_predictions(
    ...
    pd.concat([train_pred, test_pred], axis=0).sort_index().to_csv(
        output_dir / "xgb_raw_main_group_split_predictions.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def main() -> None:
    ...
    print("\n=== Group-aware CV summary for best raw-feature XGBoost ===")
    ...
    pd.DataFrame(grid_search.cv_results_).to_csv(
        output_dir / "xgb_raw_groupaware_gridsearch_cv_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    with open(output_dir / "xgb_raw_best_params.json", "w", encoding="utf-8") as f:
    ...
    print("\n=== Main group-split raw-feature XGBoost result ===")
    ...
    ]).to_csv(output_dir / "xgb_raw_main_group_split_metrics.csv", index=False, encoding="utf-8-sig")

    save_predictions(df, train_idx, test_idx, y_train_pred, y_test_pred, output_dir)

    with open(output_dir / "xgb_raw_group_model.pkl", "wb") as f:
    ...
    repeat_df.to_csv(output_dir / "xgb_raw_repeated_group_validation_metrics.csv", index=False, encoding="utf-8-sig")

    print("\nRepeated validation summary:")
    print(repeat_df[["R2", "RMSE", "MAE", "MAPE"]].describe())

    save_boxplot(repeat_df["R2"], "XGBoost raw", "Test R²", output_dir / "repeated_group_validation_R2_boxplot")
    save_boxplot(repeat_df["RMSE"], "XGBoost raw", "Test RMSE", output_dir / "repeated_group_validation_RMSE_boxplot")
