# -*- coding: utf-8 -*-
"""Random Forest model with train-only PCA and configuration-based validation.

This script trains and evaluates a Random Forest regressor for predicting the
...
PARAM_GRID = {
    ...
    "model__max_leaf_nodes": [20, 30, 40],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a Random Forest model with train-only PCA."
    )
    parser.add_argument(
        "--input",
        type=str,
...
def save_predictions(
    ...
    pd.concat([train_pred, test_pred], axis=0).sort_index().to_csv(
        output_dir / "rf_main_group_split_predictions.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def save_boxplot(values: pd.Series, ylabel: str, output_stem: Path) -> None:
    ...
    plt.boxplot([values.dropna()], labels=["Random Forest"])
    plt.ylabel(ylabel)
...
def main() -> None:
    ...
    print("\n=== Group-aware CV summary for best Random Forest pipeline ===")
    ...
    pd.DataFrame(grid_search.cv_results_).to_csv(
        output_dir / "rf_groupaware_gridsearch_cv_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    with open(output_dir / "rf_best_params.json", "w", encoding="utf-8") as f:
    ...
    print("\n=== Main group-split Random Forest result ===")
    ...
    ]).to_csv(output_dir / "rf_main_group_split_metrics.csv", index=False, encoding="utf-8-sig")
    ...
    transformed_dataframe(best_pipeline, X_train).to_csv(
        output_dir / "X_train_transformed_for_RF_check.csv",
        index=False,
        encoding="utf-8-sig",
    )
    transformed_dataframe(best_pipeline, X_test).to_csv(
        output_dir / "X_test_transformed_for_RF_check.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    with open(output_dir / "rf_group_pca_pipeline.pkl", "wb") as f:
    ...
    repeat_df.to_csv(output_dir / "rf_repeated_group_validation_metrics.csv", index=False, encoding="utf-8-sig")

    print("\nRepeated validation summary:")
    print(repeat_df[["R2", "RMSE", "MAE", "MAPE"]].describe())
    print("\nKey repeated-validation indicators:")
    print(f"R2 median   : {repeat_df['R2'].median():.4f}")
    print(f"R2 max      : {repeat_df['R2'].max():.4f}")
    print(f"MAPE median : {repeat_df['MAPE'].median():.2f} %")
